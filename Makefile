...
all: clean mydisk boot1 write_boot1 boot2 write_boot2 kernel write_kernel inGame write_inGame hexdump launchqemu
...
inGame:
	nasm -f bin $(inGame).asm -o $(inGame).bin
...
write_inGame:
	dd if=$(inGame).bin of=$(bootdisk) bs=$(blocksize) seek=$(inGamepos) count=$(inGamesize) conv=notrunc
